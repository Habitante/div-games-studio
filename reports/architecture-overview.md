...
2. **Clock and flags** -- Sets `system_clock`, `cpu_type`, `modo_de_retorno`,
   `salir_del_entorno`, `MustCreate`, etc.
...
6. **`inicializa_textos("system/lenguaje.div")`** -- Loads the localization strings.
...
9. **`inicializacion()`** -- Allocates all core buffers (see below).
10. **`inicializa_entorno()`** -- Shows copyright dialog, restores desktop session.
11. **`entorno()`** -- Enters the main event loop.
12. **Shutdown** -- Saves session, calls `finalizacion()`, `finaliza_textos()`,
    returns `modo_de_retorno` (0=quit, 1=run program, 3=test failure).

### Initialization: `inicializacion()` at line 3335

This function allocates all fundamental IDE resources:

- **Video mode detection** via `detectar_vesa()` (sets `vga_an`, `vga_al`, `big`)
- **Keyboard** via `kbdInit()`
- **Help system** via `make_helpidx()` + `load_index()`
- **Core buffers**:
  - `undo` -- undo history buffer (`undo_memory` bytes)
  - `copia` -- virtual screen copy (`vga_an * vga_al` bytes, offset by 6)
  ...
  - `barra` -- paint toolbar buffer
  - `fill_dac` -- 256-byte fill color flags
  - `error_window` -- error display buffer
  - `fondo_raton` -- mouse cursor background save buffer
...
```c
struct tventana ventana[max_windows];  // max_windows = 96
#define v ventana[0]                   // v is always the frontmost window
```

All windows are initialized with `tipo=0` (empty/inactive).

### Main Loop: `entorno()` at line 1683
...
The actual work is in `mainloop()` (line 848), which runs once per iteration:

1. **`tecla()`** -- Pumps the SDL event queue, updates `mclock`/`reloj` timestamps.
2. **Window hit-test** -- Scans `ventana[0..max_windows-1]` to find which window
...
3. **Drag-and-drop** -- Handles dragging sprites/maps between windows and the
   desktop (`arrastrar` state machine: 0=idle, 1=pending, 2=dragging, 3=done,
...
5. **Window focusing** -- If the mouse is on a non-frontmost window, it gets
   moved to `ventana[0]` (the front). Background windows (`primer_plano=0`)
   are drawn dimmed.
...
9. **`volcado_copia()`** -- Blits the virtual screen (`copia`) to the display.

### Dialog Loop: `dialog_loop()` at line 1717

Modal dialogs use a secondary event loop (`entorno_dialogo()` calls
`dialog_loop()` repeatedly until `fin_dialogo` or `salir_del_dialogo`). This
loop is simpler: it only handles the single dialog window, its title bar, and
the close button. The dialog is always `ventana[0]`.
...
SDL_PollEvent()
    |
    v
read_mouse2() [src/divmouse.c:368]
    |-- SDL_TEXTINPUT     --> ascii
...
read_mouse() [src/divmouse.c:48]
    ...
    |-- Clamps to vga_an/vga_al bounds
    |-- Updates coord_x/coord_y (map coordinates under cursor)
    v
tecla() [src/divkeybo.c:123]
...
### Shutdown: `finalizacion()` at line 3645

Frees all buffers allocated in `inicializacion()`, restores video mode via
...
```
copia (byte* vga_an*vga_al)      -- the virtual framebuffer (8-bit paletted)
    |
    v
volcado(copia)                   -- copies copia to OSDEP_buffer8->pixels
    |
    v
OSDEP_Flip()
...
### Window Structure: `struct tventana` in `src/global.h` (line 676)

Each window has:
- `tipo` -- Window type: 0=empty, 1=dialog, 2=menu, 3=palette, 4=timer,
  100=map editor, 101=FPG editor, 102=program editor, etc.
- `primer_plano` -- 0=background, 1=foreground, 2=minimized
...
- `orden` -- Z-order number
- `volcar` -- Flag indicating the window needs redrawing

The front window is always `ventana[0]`, accessed via the macro `v`.
...
1. Calling `dialogo(handler0)` where `handler0` sets up the window
2. The handler sets `v.tipo=1`, `v.titulo`, `v.an`, `v.al`
3. The handler assigns `v.paint_handler` and `v.click_handler`
4. The handler calls `_button()`, `_get()`, `_flag()` to add UI items
5. `dialogo()` enters `entorno_dialogo()` which runs `dialog_loop()` until
   `fin_dialogo=1`

### UI Items: `struct t_item` in `src/global.h` (line 653)

Three types:
- **Button** (`tipo=1`): Text label with position and centering mode.
  `_button(texto_index, x, y, center)`.
- **Text field / Get** (`tipo=2`): Text input with buffer.
  `_get(texto, x, y, width, buffer, max_len, r0, r1)`.
- **Checkbox / Flag** (`tipo=3`): Boolean toggle bound to an int variable.
  `_flag(texto, x, y, &variable)`.
...
### Menu Creation: `crear_menu()` in `src/divhandl.c` (line 1254)

Menus are built from consecutive entries in the `texto[]` array:

```c
void crear_menu(int menu) {
    v.tipo = 2;                    // Window type = menu
    v.nombre = texto[menu++];      // Icon text
    v.titulo = texto[menu];        // Title bar text
    v.an = text_len(texto[menu++]) + 23;
    v.al = 11;
    while ((char *)texto[menu]) {  // Each non-NULL entry = menu item
...
### Menu Rendering: `pinta_menu()` at line 1277

Draws each item as text on the window buffer, with separators and shortcuts.

### Menu Interaction: `actualiza_menu()` at line 1307
...
## 5. Localization: `texto[]` and `lenguaje.div`
...
### Loading: `inicializa_textos()` in `src/divlengu.c` (line 29)

1. Reads the entire file into memory
2. Calls `analiza_textos()` which walks the buffer character by character
3. Numbers update `numero` (the current index)
4. Quoted strings are decoded in-place, null-terminated, and stored as
   `texto[numero]` pointers

### Usage

All UI text references go through `texto[N]` where N is the index from
...
Examples:
- `texto[34]` = Window title ("DIV Games Studio")
- `texto[100]` through `texto[199]` = Button labels
- `texto[200]` through `texto[299]` = Compiler messages
- `texto[500]+` = Error messages
...
- `Codigo` -- Graphic ID (for FPG association)
- `descripcion[32]` -- Description text
