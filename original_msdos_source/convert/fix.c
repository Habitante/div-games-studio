...
void main(int argc, char * argv[])
{
  ...
  printf("FIX \"Arreglador\" de archivos PCM y WAV (c) 1999 Freeware");

  if (argc<2) {
    printf("\n\nUso: FIX.EXE <Archivos> ฤู");
    exit(0);
  }
  ...
  while(!rc) {
    ...
    if( IsWAV(full) ) wav=1; else wav=0;
...
int Save(byte *byte_ptr, int length, FILE *dstfile)
{
