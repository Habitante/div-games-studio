...
WORD IPX_ListendPacket(WORD NECB)
{
  ...
  if(IPX_ECB[NECB]->InUseFlag)
          return((WORD)0);
  if(!IPX_Packet[NECB]->Atendido)
          return((WORD)1);
...
//    j=CheckLookPackets();
//    if ((IPX_Packet[j]->Comando==partida_red.game_id))
...
//  if (_fmemcmp(IPX_WorkAddress.NodeAddress,IPX_MyAddress.NodeAddress,6))
//  if (IPX_Packet[OLDESTECB]->Comando>=10000)
//    return 0;
...
/*
        if (partida_red.servidor)
          SRV_Packet(j,IPX_Packet[OLDESTECB]->Comando,WBuffer,IPX_Packet[OLDESTECB]->Longitud);
        else
          NOD_Packet(1,IPX_Packet[OLDESTECB]->Comando,WBuffer,IPX_Packet[OLDESTECB]->Longitud);
*/
      }
...
//      delete_command(NET_SERVER);
      break;
    case NET_EXIT:
      delete_player(IPX_WorkAddress);
//      delete_command(NET_EXIT);
      break;
    case NET_ADDUSER:
      _fmemcpy(IPX_WorkAddress,IPX_Packet[OLDESTECB]->Datos,sizeof(LocalNodeAddress));
//      printf("Direccion: %s\n",ShowInternalAddress(IPX_WorkAddress));
...
//      printf("Direccion: %s\n",ShowInternalAddress(IPX_WorkAddress));
      ...
      ClearECBS();
//      delete_command(NET_JOIN);
      break;
/*
    case NET_ACCEPTJOIN:
//      printf("Se acepta la inclusion\n");
//      _fmemcpy(IPX_WorkAddress.NodeAddress,IPX_ECB[j]->InmediateLocalAddress,6);
...
*/
    case NET_LOOKINGFOR:
      if (partida_red.num_players>=partida_red.max_players ||
          !partida_red.servidor)
        break;
//      printf("Comando looking for detectado\n");
      IPX_Packet[0]->Atendido=0;
