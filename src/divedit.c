...
void program2(void) {
  ...
  if (wmouse_x != -1 && v.state) {
    if ((mouse_b & 1) && mouse_y >= v.y + 18 * big2 && mouse_x >= v.x + 2 * big2) {
      ...
      if (n >= v.prg->first_line && n < v.prg->first_line + v.prg->al && m >= v.prg->first_column &&
          m < v.prg->first_column + v.prg->an) {
  ...
  if (mouse_graf == 9 && (mouse_b & 1) && wmouse_x != -1) {
    if (!(v.buttons & 4)) {
      wput(v.ptr, an, al, an - 9, al - 17, -42);
      v.buttons |= 4;
    } else {
      retrace_wait();
      retrace_wait();
    ...
    wput(v.ptr, an, al, an - 9, al - 17, -40);
    v.buttons ^= 4;
    v.redraw++;
  }
  ...
  if (mouse_graf == 11 && (mouse_b & 1) && wmouse_x != -1) {
    if (!(v.buttons & 16)) {
      wput(v.ptr, an, al, an - 17, al - 9, -54);
      v.buttons |= 16;
    } else {
      retrace_wait();
      retrace_wait();
    ...
    wput(v.ptr, an, al, an - 17, al - 9, -52);
    v.buttons ^= 16;
    v.redraw++;
  }
  ...
  if (bloque_edit == 1) { // Single-line block edit
    ...
    if ((shift_status & 3) && !(shift_status & 12))
      ...
      case 0:
        break;
      case 77:
        if (kcol2 + 1 == v.prg->column)
          kcol2++;
        else if (kcol1 == v.prg->column)
          kcol1++;
        if (kcol1 > kcol2)
          f_unmark();
        f_right();
  ...
  if (kbloque == 2)
    if (kprg == v.prg) {
      if (in_block()) {
        kbloque = 1;
        kini = v.prg->lptr;
        if ((kcol1 = v.prg->column) > strlen(v.prg->l))
          kcol1 = strlen(v.prg->l) + 1;
        ...
        kfin = v.prg->lptr;
        kcol2 = v.prg->column;
        if (kcol2 > strlen(v.prg->l))
          kcol2 = strlen(v.prg->l) + 1;
      }
    } else {
      for (n = 0; n < max_windows; n++)
    ...
    kbloque = 2;
    kfin = v.prg->lptr;
    if ((kcol2 = v.prg->column) > strlen(v.prg->l))
      kcol2 = strlen(v.prg->l) + 1;
...
void f_cut(int borrar) { // 0-Copy, 1-Cut, 2-Delete
  ...
  if (kbloque == 0)
    ...
    f_mark();
  write_line();
  if (kcol1 > linelen(kini))
    kcol1 = linelen(kini) + 1; // ->cr
  if (kcol2 > linelen(kfin))
    kcol2 = linelen(kfin) + 2; // ->lf

  if (kcol1 == 1 && kcol2 == linelen(kfin) + 2)
...
void f_paste_block(void) {
  ...
  if (papelera != NULL) {
    ...
    kfin = ini + espacios + lon_papelera - 1;
    kcol2 = 1; // Set kfin
    while (kfin != v.prg->buffer && *kfin != cr) {
      kcol2++;
      kfin--;
    }
    if (kfin != v.prg->buffer && *kfin == cr) {
      kfin += 2;
      kcol2 -= 2;
    }
...
void f_delete_char(void) {
  ...
  if (strlen(v.prg->l) < v.prg->column) { // Join two lines
    ...
    if (kini == v.prg->lptr && kcol1 > v.prg->column)
      kcol1--;
    if (kfin == v.prg->lptr) {
      if (kcol2 > v.prg->column)
        kcol2--;
      else if (kcol2 == v.prg->column) {
        if (kini == v.prg->lptr && kcol1 == kcol2)
          kbloque = 0;
        else if (kcol2 == 1) {
          retreat_lptr();
          kfin = v.prg->lptr;
...
void f_enter(void) {
  ...
  if ((lon = strlen(v.prg->l)) < v.prg->column) { // Normal enter
    ...
    if (kini == v.prg->lptr && kcol1 >= v.prg->column) {
      kini += n;
      kcol1 = kcol1 - v.prg->column + t + 1;
    }
    if (kfin == v.prg->lptr && kcol2 >= v.prg->column) {
      kfin += n;
      kcol2 = kcol2 - v.prg->column + t + 1;
    }
...
void f_delete(void) {
  if (kini == v.prg->lptr && kfin == v.prg->lptr)
    kbloque = 0;
  else if (kini == v.prg->lptr)
    kcol1 = 1;
  ...
  do {
    ...
    if (kbloque == 0 || kprg != v.prg || si < kini || si > kfin) { // Outside the block
      if (si > v.prg->buffer + v.prg->file_len)
        while (n--) {
          memset(di, ce01, v.an - 12 * big2);
          di += v.an;
        }
      else
        while (n--) {
          memset(di, ce1, v.an - 12 * big2);
          di += v.an;
        }
    } else if (si > kini && si < kfin) { // Inside the block
      while (n--) {
        memset(di, ce4, v.an - 12 * big2);
        di += v.an;
      }
      ...
      while (ancho--) {
        ...
        di -= v.an * editor_font_height - editor_font_width;
        x++;
      }
    }
    di = old_di;
    ...
    if (alto == 1 && big && (editor_font_height * v_prg->al & 1)) { // Fix odd-sized window
      if (*di == ce01)
        memset(di + v.an * editor_font_height, ce01, v.an - 24);
      else
        memset(di + v.an * editor_font_height, ce1, v.an - 24);
    }
...
void _parcial(void) {
  ...
  while (csource < v.prg->lptr)
    color_lex();
  numrem = 0;
  csource = (byte *)v.prg->l;
  fill_color_line();
  color_cursor = colin[v.prg->column - 1];
...
void info_bar(void) {
  ...
  wwrite_in_box(v.ptr, an, an - 19, al, 3, 10, 0, (byte *)num, c3);
  ancho = text_len((byte *)num) + 1;
  itoa(v.prg->column, num + 1, 10);
  num[0] = ',';
  wwrite_in_box(v.ptr, an, an - 19, al, 3 + ancho, 10, 0, (byte *)num, c3);
  ancho += text_len((byte *)num) + 1;
}
...
void resize(void) {
  ...
  do {
    ...
    if (v.prg->an < 4 * big2) {
      v.prg->an = 4 * big2;
      mx = _mx + (v.prg->an - _an) * editor_font_width;
    }
    if (v.prg->al < 2 * big2) {
      v.prg->al = 2 * big2;
      my = _my + (v.prg->al - _al) * editor_font_height;
    }
    if (v.prg->an > 80 * 2) {
      v.prg->an = 80 * 2;
      mx = _mx + (v.prg->an - _an) * editor_font_width;
...
void maximize(void) {
  ...
  if (v.prg->an != _an || v.prg->al != _al) { // *** Maximize ***
    ...
    if ((new_block = (byte *)realloc(v.ptr, v.an * v.al)) != NULL) {
      ...
      test_cursor();
      repaint_window();
      v.x = vga_width / 2 - v.an / 2;
      v.y = vga_height / 2 - v.al / 2;
      // place_window(1,&v.x,&v.y,v.an,v.al);
      if (exploding_windows)
        extrude(v.prg->old_x, v.prg->old_y, _an, _al, v.x, v.y, v.an, v.al);
      v.redraw = 2;
    ...
    if ((new_block = (byte *)realloc(v.ptr, v.an * v.al)) != NULL) {
      ...
      if (exploding_windows)
        extrude(_x, _y, _an2, _al2, v.x, v.y, v.an, v.al);
      v.redraw = 2;
...
int in_block(void) {
  ...
  if (kbloque & 1)
    ...
    if (v.prg->lptr > kini && v.prg->lptr < kfin)
      return (1);
    else {
      if (v.prg->lptr == kini)
        col0 = kcol1;
...
void text_cursor(void) {
  ...
  if (modo_cursor) {
    ...
    alto = editor_font_height / 2;
    di = old_di + v.an * (editor_font_height - alto);
  }
...
void write_line(void) {
  ...
  if (ini < v.prg->buffer + old_lon) {
    ...
    memmove(fin, ini, lon);
    if (kini > ini && kini < ini + lon)
      kini += fin - ini;
    if (kfin > ini && kfin < ini + lon)
      kfin += fin - ini;
  }
...
void replace_text(void) {
  ...
  do {
    ...
    if (encontrado) {
      ...
      kcol1 = v.prg->column;
      kfin = v.prg->lptr;
      kcol2 = v.prg->column + strlen(buscar2) - 1;
      n = strlen(buscar2);
      while (n--)
        f_right();
      n = strlen(buscar2);
...
void replace0(void) {
  ...
  v.type = 1;
  v.title = texts[190];
  v.an = x4 + text_len(texts[101] + 1) + 7;
  v.al = 29;
...
void process_list2(void) {
  ...
  if (wmouse_in(122 + 132, 93 + 40, 9, 9)) {
    if (mouse_b & 1) {
      if (lp_boton == 0) {
        wput(ptr, an, al, 123 + 132, 94 + 40, -42);
        lp_boton = 2;
        if (lp_select + 1 < lp_num) {
          if (lp_ini + 15 == ++lp_select)
            lp_ini++;
      ...
      wput(ptr, an, al, 123 + 132, 94 + 40, -40);
      lp_boton = 0;
      v.redraw = 1;
    }
    mouse_graf = 9;
...
void goto_error(void) {
  ...
  if (linea_error > v.prg->num_lines) {
    linea_error = v.prg->num_lines;
    m = 1;
  } else
    m = 0;
  if (linea_error < 1)
  ...
  if (v.prg->line <= v.prg->first_line)
    retreat_vptr();
  if (v.prg->line - v.prg->first_line >= v.prg->al - 1)
    advance_vptr();
