...
int fmt_save_map(byte *mapa, FILE *f) {
  ...
  x = map_width;
  fwrite(&x, 2, 1, f); // +008 Width
  x = map_height;
  fwrite(&x, 2, 1, f); // +010 Height
  y = window[v_window].mapa->fpg_code;
...
void fmt_load_bmp(byte *buffer, byte *mapa, int vent) {
  ...
  if (InfoHeader.biBitCount == 8) {
    if (InfoHeader.biClrUsed)
      memcpy(Bmpdac, pSrc, 4 * InfoHeader.biClrUsed);
    else
      memcpy(Bmpdac, pSrc, 4 * 256);
  } else if (InfoHeader.biBitCount == 4) {
    if (InfoHeader.biClrUsed)
      memcpy(Bmpdac, pSrc, 4 * InfoHeader.biClrUsed);
    else
      memcpy(Bmpdac, pSrc, 4 * 16);
  }
...
int fmt_load_jpg(byte *buffer, byte *mapa, int vent, int img_filesize) {
  ...
  buffer_scanline = (*cinfo.mem->alloc_sarray)((j_common_ptr)&cinfo, JPOOL_IMAGE, row_stride, 1);

  while (cinfo.output_scanline < cinfo.output_height) {
    jpeg_read_scanlines(&cinfo, buffer_scanline, 1);
    memcpy(&mapa[(cinfo.output_scanline - 1) * cinfo.output_width], buffer_scanline[0], row_stride);
  }
