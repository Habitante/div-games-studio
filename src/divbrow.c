...
void create_thumbs(void) {
  if (opc_img[v_thumb]) {
    do {
      ...
      {
      case 2:
        create_thumb_MAP(&file_list_br);
        break;
      case 3:
        create_thumb_PAL(&file_list_br);
        break;
      case 5:
        create_thumb_FNT(&file_list_br);
        break;
      case 6:
        create_thumb_IFS(&file_list_br);
        break;
      case 7:
        create_thumb_PCM(&file_list_br);
        break;
      }
      if (num > -1) {
        if (thumb[num].ptr != NULL && thumb[num].status == 0) {
          show_thumb(&file_list_br, num);
          break;
  ...
  if (l->total_items) {
    ...
    if (state == 1) { // Read a new thumbnail
      ...
      if (strchr(l->list + (l->item_width * num), '.') != NULL &&
        ...
        fseek(f, 0, SEEK_SET);
        if (thumb[num].filesize <= FILE_CHUNK)
      ...
      if ((f = fopen(l->list + (l->item_width * num), "rb")) != NULL) {
        ...
        if (thumb[num].filesize - thumb[num].status > incremento) {
          ...
          if (fread(thumb[num].ptr + thumb[num].status, 1, thumb[num].filesize - thumb[num].status,
                    f) == thumb[num].filesize - thumb[num].status) {
            thumb[num].status = thumb[num].filesize;
          } else {
            free(thumb[num].ptr);
            thumb[num].ptr = NULL;
...
void load_letter(uint8_t char_code) {
  ...
  if (fread(&map_height, 2, 1, fifs) < 1)
    error = 1;
  if (fread(&despY, 2, 1, fifs) < 1)
    error = 1;
  if (fread(&map_width, 2, 1, fifs) < 1)
    error = 1;
  if (fread(&pixels, 2, 1, fifs) < 1)
    error = 1;
  if (error) {
    map_width = 0;
    return;
...
void browser0(void) {
  ...
  if (v_mode == 1)
    *input = 0;
  if (v_mode == 2)
    div_strcpy(input, sizeof(input), input2);

  file_list_br.created = 0;
  dir_list_br.created = 0;
  lunidadesbr.created = 0;
  lextbr.created = 0;

  n = 0;
  while (drives[n]) { // Create the drive list <X:>
    *(drives_buf + DRIVE_WIDTH * n) = '<';
    *(drives_buf + DRIVE_WIDTH * n + 1) = drives[n];
    *(drives_buf + DRIVE_WIDTH * n + 2) = ':';
    *(drives_buf + DRIVE_WIDTH * n + 3) = '>';
    *(drives_buf + DRIVE_WIDTH * n + 4) = 0;
    n++;
  }
  ...
  if (opc_img[v_thumb]) {
    ...
    case 5:
    case 6:
      file_list_br.columns = 2;
      file_list_br.w = 103;
      break;
    default:
      file_list_br.columns = 4;
      file_list_br.w = 51;
      break;
    }
    file_list_br.lines = 4;
    file_list_br.h = 31;
  } else {
    file_list_br.columns = 3;
    file_list_br.w = 68;
    file_list_br.lines = 14;
    file_list_br.h = 8;
  }
...
void browser2(void) {
  ...
  update_listbox_br(&file_list_br);
  update_listbox(&dir_list_br);
    ...
    if (file_list_br.zone >= 10) {
      if (v_type != 6 && !v_mode) {
        if (shift_status & 3) // SHIFT
        {
          ...
          if (selected > ini_tagged)
            pos1 = ini_tagged, pos2 = selected;
          else
            pos2 = ini_tagged, pos1 = selected;
          for (n = pos1; n <= pos2; n++) {
            thumb[n].tagged = 1;
            num_taggeds++;
          }
      ...
      if (strcmp(input, full) || ((v_thumb == 7 || v_type == 16) && opc_pru)) {
        div_strcpy(input, sizeof(input), full);
        browser1();
        v.redraw = 1;
#ifdef MIXER
...
void paint_listbox_br(struct t_listboxbr *l) {
  ...
  if (n > l->lines * l->columns)
    n = l->lines * l->columns;
  while (n > 0)
    show_thumb(l, l->first_visible + --n);
}
