...
void menu_programas1(void) { paint_menu(900); }

void menu_programas2(void) {
  FILE * f;
  int n;

  if (find_program_window()) update_menu(900,1,0);
  else update_menu(900,3,99);
  ...
  if ((prev_mouse_buttons&1) && !(mouse_b&1)) {
    ...
    switch (v.state) {
      ...
      case 1: // Nuevo ...
        ...
        if (v_finished) {
          if (!v_exists) v_accept=1;
          else {
            v_title=(char *)texto[187]; v_text=input;
      ...
      case 3:
        if ((n=find_program_window())) {
          v_title=(char *)texto[188]; v_text=(char *)ventana[n].title;
          show_dialog(aceptar0);
          if (v_accept) {
            move(0,n);
            close_window();
      ...
      case 5:
        if ((n=find_program_window())) {
          save_prg_buffer(n);
          v_mode=1; v_text=(char *)texto[906];
          show_dialog(browser0);
          if (v_finished) {
            if (v_exists) {
              v_title=(char *)texto[187]; v_text=input;
      ...
      case 6: new_window(menu_edicion0); break;

      case 8:
        if ((n=find_program_window())) {
          save_prg_buffer(n);
          source_ptr=ventana[n].prg->buffer;
          source_len=ventana[n].prg->file_len;
          v_window=n;
          run_mode=0;
          ...
          if(numero_error!=-1) {
            goto_error();
            if (v_help) help(500+numero_error);
          } else if (v_help) help(599);
        } break;
        ...
        if ((n=find_program_window())) {
          ...
          if(numero_error!=-1) {
            ...
            if (f!=NULL) {
              ...
              v_text=(char *)texto[238]; show_dialog(err0);
            }
          }
        } break;
...
void menu_edicion1(void) { paint_menu(950); }

void menu_edicion2(void) {
  int n,m;

  if (find_program_window()) update_menu(950,1,0);
  else update_menu(950,0,99);
  ...
  if ((prev_mouse_buttons&1) && !(mouse_b&1)) {
    ...
    switch (v.state) {
      ...
      case 1: // Delete line
        if ((n=find_program_window())) { wup(n);
          f_delete();
          _completo(); v.redraw=2; wdown(n); flush_window(n);
        } break;
...
void menu_paletas1(void) { paint_menu(775); }

void menu_paletas2(void) {
  update_menu(775,1,0); if ((prev_mouse_buttons&1) && !(mouse_b&1)) {
    switch (v.state) {
      case 1: LoadPal(); break;
      case 2: SaveAsPal(); break;
      case 3: EditPal(); break;
      case 4: sort_palette(); break;
      case 5: merge_palette(); break;
      case 6: new_window(paleta0); break;
    }
  }
}
...
void menu_mapas0(void) { create_menu(800);
  v.paint_handler=menu_mapas1;
  v.click_handler=menu_mapas2; }

void menu_mapas1(void) { paint_menu(800); }

void menu_mapas2(void) {
  int n,m;

  if (find_map_window()) update_menu(800,1,0);
  else update_menu(800,3,9);
  ...
  if ((prev_mouse_buttons&1) && !(mouse_b&1)) {
    ...
    switch (v.state) {
      ...
      case 4: // Cerrar todos los mapas (maximizados)
        ...
        show_dialog(aceptar0);
        if (v_accept) {
          for (n=0;n<max_windows;n++) if (ventana[n].type==100 && ventana[n].foreground!=2) {
            move(0,n); close_window(); n=-1;
          }
        } break;
      ...
      case 5: // Guardar mapa
        if ((n=find_and_load_map())) {
          ...
          if (ventana[n].mapa->path[0]==0) {
            v_mode=2;
            strcpy(input2,(char *)ventana[n].title);
            goto casi_no_tiene_nombre;
          }
          mouse_graf=3; flush_copy(); mouse_graf=1;
          strcpy(tipo[v_type].path,ventana[n].mapa->path);
      ...
      casi_no_tiene_nombre:
          v_text=(char *)texto[807];
          show_dialog(browser0);
          if (v_finished) {
            if (v_exists) {
              v_title=(char *)texto[52]; v_text=input;
              show_dialog(aceptar0);
            } else v_accept=1;
            if (v_accept) {
              mouse_graf=3; flush_copy(); mouse_graf=1;
  ...
  if (v_finished) {
    if ((f=fopen(full,"rb"))!=NULL) { // Se ha elegido uno
      if (fread(cwork,1,8,f)==8) {
        ...
        } else { v_text=(char *)texto[46]; show_dialog(err0); }
      } else { fclose(f); v_text=(char *)texto[44]; show_dialog(err0); }
...
int find_pcm_window(void) {
  ...
  for (m=1;m<max_windows;m++) if (ventana[m].type==105 && ventana[m].state) {
    n=m; break;
  } return(n);
}
...
int find_mod_window(void) {
  ...
  for (m=1;m<max_windows;m++) if (ventana[m].type==107 && ventana[m].state) {
    n=m; break;
  } return(n);
}
...
int find_font_window(void) {
  ...
  for (m=1;m<max_windows;m++) if (ventana[m].type==104 && ventana[m].state) {
    n=m; break;
  } return(n);
}
...
int find_fpg_window(void) {
  ...
  for (m=1;m<max_windows;m++) if (ventana[m].type==101 && ventana[m].state) {
    n=m; break;
  } return(n);
}
...
int find_map_window(void) {
  ...
  for (m=1;m<max_windows;m++) if (ventana[m].type==100 && ventana[m].state) {
    n=m; break;
  } return(n);
}
...
int find_and_load_map(void) {
  ...
  for (m=1;m<max_windows;m++) if (ventana[m].type==100 && ventana[m].state) {
    ...
    v_window=n; v_map=ventana[n].mapa; map=v_map->map;
    map_width=v_map->map_width; map_height=v_map->map_height;
  } return(n);
}
...
void menu_graficos2(void) {
  ...
  if (find_fpg_window() && find_map_window()) update_menu(825,1,0);
  else if (find_fpg_window()) update_menu(825,9,9);
  else if (find_map_window()) update_menu(825,3,8);
  else update_menu(825,3,9);
          ...
          if (v_finished) {
            if (v_exists) {
              ...
              if (!strcmp(full,(char *)Fpg->ActualFile)) {
                v_text=(char *)texto[418];
                show_dialog(err0);
                v_accept=0;
              } else {
                v_title=(char *)texto[82]; v_text=input;
                show_dialog(aceptar0);
              }
...
void menu_fuentes2(void) {
  ...
  if (find_font_window()) {
    if (find_map_window()) update_menu(850,1,0);
    else update_menu(850,6,6);
  } else {
    if (find_map_window()) update_menu(850,3,5);
    else update_menu(850,3,6);
  }
  ...
  if ((prev_mouse_buttons&1) && !(mouse_b&1)) {
    switch (v.state) {
      ...
      case 6:
        if ((n=find_and_load_map())) print_fontmap();
        break;
    }
  }
}
...
void print_fontmap(void) {
  ...
  v_text=(char *)texto[264]; show_dialog(browser0);

  if (!v_finished) return;

  if (v_exists) {
    v_title=(char *)texto[75]; v_text=(char *)texto[76];
    show_dialog(aceptar0); if(!v_accept) return;
  }
  ...
  for(x=1;x<map_width-1;x++) {
    for(y=1;y<map_height-1;y++) {
      ...
      if (mapa(x,y)!=color && mapa(x+1,y+1)==color && mapa(x+1,y)==color && mapa(x,y+1)==color) {
        ...
        if ((x+=2)>=map_width) { goto end_bucle; } y=0;
      }
    }
  }
  ...
  if (chars!=256) { v_text=(char *)texto[243]; show_dialog(err0); free(buffer); return; }
  ...
  if (*(int*)(buffer+1356+'\xa4'*16)) gencode|=16;

  memcpy(buffer+1352,&gencode,4);

  if ((f=fopen(FontPathName,"wb"))==NULL) {
    v_text=(char *)texto[242]; show_dialog(err0);
    free(buffer); return;
  }
  ...
  for (x=-1,y=1;y<max_windows;y++) if (ventana[y].type==104 && !strcmp((char *)ventana[y].title,FontName)) x=y;
...
void generate_fontmap(void) {
  ...
  map_width=1; map_height=0;

  for (n=0;n<256;n++) {
    if (*(p+n*4)==0 || *(p+n*4+1)==0) map_width+=2; else {
      map_width+=*(p+n*4)+1;
      if (map_height<2+*(p+n*4+1)+*(p+n*4+2)) map_height=2+*(p+n*4+1)+*(p+n*4+2);
  ...
  if (v_map!=NULL && v_map->map!=NULL) {
    ...
    for (x=1,n=0;n<256;n++) {
      generate_character(v_map->map+map_width+x,*(p+n*4),*(p+n*4+1),*(p+n*4+2),FntAux+*(p+n*4+3));
...
void menu_sonidos1(void) { paint_menu(925); }
...
void menu_sonidos2(void) {
  ...
  if (find_pcm_window()) update_menu(925,1,0);
  else update_menu(925,3,6);
  ...
  if ((prev_mouse_buttons&1) && !(mouse_b&1)) {
    switch (v.state) {
      ...
      case 1:
        ...
        if (!SoundActive) {
          if ( SoundError ) {
            v_text=(char *)texto[549]; show_dialog(errhlp0);
            if (v_accept) help(2008);
          } else {
            v_text=(char *)texto[548]; show_dialog(errhlp0);
            if (v_accept) help(2009);
          }
          return;
        } else if (!SoundActive) {
          v_text=(char *)texto[575]; show_dialog(err0);
          return;
        }
      ...
      case 3:
        if ((n=find_pcm_window())) {
          v_title=(char *)texto[338]; v_text=(char *)ventana[n].title;
          show_dialog(aceptar0);
          if (v_accept) { move(0,n); close_window(); }
        } break;

      case 4:
        if ((n=find_pcm_window())) {
          if (strchr((const char *)ventana[n].title,' ')!=NULL) goto no_tiene_nombre_sonido;
          mouse_graf=3; flush_copy(); mouse_graf=1;
      ...
      no_tiene_nombre_sonido:
        if ((n=find_pcm_window())) {
          v_type=7; v_mode=1; v_text=(char *)texto[339];
          show_dialog(browser0);
          if (v_finished) {
            if (v_exists) {
              v_title=(char *)texto[340]; v_text=input;
              show_dialog(aceptar0);
            } else v_accept=1;
            if (v_accept) {
              mouse_graf=3; flush_copy(); mouse_graf=1;
      ...
      case 6:
        if ((n=find_pcm_window())) {
          ...
          v_type=7; v_text=(char *)texto[349];

//          judas_stopsample(0);

          show_dialog(EditSound0);
          move(0,n); call((voidReturnType )v.paint_handler); v.redraw=1;
          PasteNewSounds();
        }
        break;
      ...
      case 9:
        if ((n=find_mod_window())) {
          v_title=(char *)texto[513]; v_text=(char *)ventana[n].title;
          show_dialog(aceptar0);
          if (v_accept) { move(0,n); close_window(); }
        } break;
        break;
...
void menu_sistema1(void) { paint_menu(875); }
...
void menu_sistema2(void) {
  update_menu(875,1,0); if ((prev_mouse_buttons&1) && !(mouse_b&1)) {
    switch (v.state) {
      case 1: show_clock(); break;
      case 2: muestra_papelera(); break;
      case 3: calculadora(); break;
      case 4: show_dialog(Vid_Setup0); break;
      case 5: show_dialog(Tap_Setup0); break;
      case 6:
        show_dialog(Cfg_Setup0);
        Cfg_Setup_end();
        break;
      case 7: show_dialog(MemInfo0); break;
      case 8: show_dialog(about0); break;
      case 9:
        v_title=(char *)texto[40]; v_text=NULL; show_dialog(aceptar0);
        if (v_accept) { exit_requested=1; } break;
    }
  }
}
...
void update_menu(int menu,int min,int max) {
	...
	if (n!=v.state) {
		...
		if (v.state) {
			...
			p=texto[menu+abs(v.state)];
			if (*p=='-')
				p++;

			if ((q=(byte *)strchr((const char *)p,'['))!=NULL) {
				*q=0; wwrite(ptr,an,al,3,2+abs(v.state)*9,0,p,c3);
				*q='[';
				wwrite(ptr,an,al,an-4,2+abs(v.state)*9,2,q,c1);
				wwrite(ptr,an,al,an-5,2+abs(v.state)*9,2,q,c3);
			} else wwrite(ptr,an,al,3,2+abs(v.state)*9,0,p,c3);
		}
...
void mapa0(void) {
  ...
  if ((v.al=v.mapa->map_height)<16*big2) v.al=(8+12)*big2;
  else {
  ...
  v.mapa->zoom_cx=v.mapa->map_width/2; v.mapa->zoom_cy=v.mapa->map_height/2;
  x=(v.mapa->zoom_cx-vga_width/2);
  if (x<0) x=0; else if (x+vga_width>v.mapa->map_width) x=v.mapa->map_width-vga_width;
  y=(v.mapa->zoom_cy-vga_height/2);
  if (y<0) y=0; else if (y+vga_height>v.mapa->map_height) y=v.mapa->map_height-vga_height;
...
void mapa1(void) {
  ...
  if (v.mapa->map_width<48*big2) di+=(24*big2-v.mapa->map_width/2)/2;
  if (v.mapa->map_height<16*big2) di+=((8*big2-v.mapa->map_height/2)/2)*andi;

  ansi=v.mapa->map_width; si=v.mapa->map;
  ...
  if ((an=v.mapa->map_width/2)>maxan) {
    ...
    if (x<0) x=0; else if (x+maxan*2>v.mapa->map_width) x=v.mapa->map_width-maxan*2;
    si+=x;
  }

  if ((al=v.mapa->map_height/2)>maxal) {
    al=maxal; y=(v.mapa->zoom_cy-maxal);
    if (y<0) y=0; else if (y+maxal*2>v.mapa->map_height) y=v.mapa->map_height-maxal*2;
    si+=y*ansi;
  }
...
void calculate_foreground(void) {
  ...
  for (n=0;n<max_windows;n++) if (ventana[n].type && ventana[n].foreground!=2) {
    ventana[n].foreground=1;
    for (m=0;m<n;m++) if (ventana[m].type) {
...
void mapa2(void) {
  ...
  if ((mouse_b&1)) {
    ...
    map_width=v.mapa->map_width; map_height=v.mapa->map_height;
    ...
    if (ventana[n].type && ventana[n].foreground==1) {
      hidden[n]=1; ventana[n].foreground=0;
    } else hidden[n]=0;
    ...
    if (dragging==4) {
      mouse_b=0; old_draw_mode=draw_mode; draw_mode=190;
    } else do { read_mouse(); } while (mouse_b&1);

    highlight_background=0; current_mouse=21+paint_cursor*4;
    ...
    do {
     ...
     draw_mode-=100; switch(draw_mode) {
      ...
      if (draw_mode<100) { put_bar(2,10,45); } blit_mouse();
      do { read_mouse(); } while (mouse_b || key(_ESC));
    } while (draw_mode>=100); mouse_graf=1;

    free(selection_mask); selection_mask=NULL;
    ...
    if (draw_mode<90) draw_mode+=100; else draw_mode=old_draw_mode;
    ...
    for (n=1;n<max_windows;n++) if (hidden[n]) ventana[n].foreground=1;
    ...
    mapa1(); update_box(0,0,vga_width,vga_height);
    if (undo_index!=_undo_index) v.mapa->saved=0;

    n=0; while (n<num_windows_to_create) {
...
void copyright2(void) {
  ...
  if (v.active_item==1) { nueva_sesion=1; end_dialog=1; }
}
...
void errhlp2(void) {
  _process_items();
  if (v.active_item==0) { end_dialog=1; }
  if (v.active_item==1) { v_accept=1; end_dialog=1; }
}
...
void test2(void) {
  _process_items();
  if (v.active_item==0) { exit_requested=0; return_mode=0; end_dialog=1; }
  if (*system_clock>old_system_clock+180) end_dialog=1;
}

void test0(void) {
  v.type=1; v.title=texto[384];
  v_text=(char *)texto[383];
  v.an=text_len((byte *)v_text)+9; v.al=38;
  v.paint_handler=test1;
  v.click_handler=test2;
  _button(100,v.an/2,v.al-14,1);

  exit_requested=1; return_mode=3;
...
void aceptar0(void) {
  ...
  if (v_title!=NULL) { v.title=(byte *)v_title;
    if (text_len((byte *)v_title)+14>v.an) v.an=text_len((byte *)v_title)+14; }
  if (v_text!=NULL) { v.al=38;
    if (text_len((byte *)v_text)+6>v.an) v.an=text_len((byte *)v_text)+6;
  } else v.al=29;
...
void aceptar1(void) {
  int an=v.an/big2,al=v.al/big2;
  _show_items();
  if (v_text!=NULL) wwrite(v.ptr,an,al,an/2,12,1,(byte *)v_text,c3);
}
...
void TratarPaleta0(void) {
  ...
  v.title=(byte *)(v_title=(char *)texto[568]);
...
void TratarPaleta3(void) {
  if(v_accept && OpcPal[1]) v_accept=2;
  if(v_accept && OpcPal[2]) v_accept=3;
}
...
void paint_listbox(struct t_listbox * l) {
  ...
  } else if (wmouse_in(l->x+l->an-1,l->y,9,9)) l->zone=2;
  else if (wmouse_in(l->x+l->an-1,l->y+l->al-9,9,9)) l->zone=3;
...
void create_listbox(struct t_listbox * l) {
  ...
  if (!l->created) {
    if (!l->an) l->an=(l->item_width-1)*6+3;
...
void update_listbox(struct t_listbox * l) {
  ...
  if ((mouse_b&8 && l->zone>0) || (l->zone==2 && ((mouse_b&1)||(v_pause&&!(mouse_b&1)&&(prev_mouse_buttons&1))))) {
    if (!v_pause||(v_pause&&!(mouse_b&1)&&(prev_mouse_buttons&1))) {
        if ((prev_mouse_buttons&1)&&!v_pause) { retrace_wait(); retrace_wait(); }
        if (l->first_visible) {
          l->first_visible--; paint_listbox(l); v.redraw=1; }
    }
    wput(ptr,an,al,l->x+l->an,l->y+1,-41); l->buttons|=1; v.redraw=1;
  } else if (l->buttons&1) {
    wput(ptr,an,al,l->x+l->an,l->y+1,-39); l->buttons^=1; v.redraw=1;
  }

  if ((mouse_b&4 && l->zone>0) || (l->zone==3 && ((mouse_b&1)||(v_pause&&!(mouse_b&1)&&(prev_mouse_buttons&1))))) {
    if (!v_pause||(v_pause&&!(mouse_b&1)&&(prev_mouse_buttons&1))) {
      if ((prev_mouse_buttons&1)&&!v_pause) { retrace_wait(); retrace_wait(); }
    ...
    wput(ptr,an,al,l->x+l->an,l->y+l->al-8,-42); l->buttons|=2; v.redraw=1;
  } else if (l->buttons&2) {
    wput(ptr,an,al,l->x+l->an,l->y+l->al-8,-40); l->buttons^=2; v.redraw=1;
  }
  ...
  if (l->zone==4 && (mouse_b&1)) {
    ...
    if (l->total_items>l->visible_items) {
      ...
      if (n!=l->first_visible) { l->first_visible=n; paint_listbox(l); }
    } paint_slider(l); v.redraw=1;
  } else {
    if (l->total_items<=l->visible_items) n=l->s0;
    else {
      n=l->total_items-l->visible_items;
...
void nuevo_mapa0(void) {
  v.type=1; v.title=texto[132];
  v.an=126; v.al=14+y_nm;
  v.paint_handler=nuevo_mapa1;
  v.click_handler=nuevo_mapa2;
  lmapsizes.created=0; lmapsizes.total_items=9;
  map_width=vga_width; map_height=vga_height;
  itoa(map_width,mancho,10); itoa(map_height,malto,10);
  ...
  if ((mouse_b&1) && !(prev_mouse_buttons&1)) {
    if (lmapsizes.zone>=10) {
      ...
      *strchr(&mapsizes[n*9],'x')=0;
      ...
      itoa(map_width,mancho,10); itoa(map_height,malto,10);
      call((voidReturnType )v.paint_handler); update_listbox(&lmapsizes);
      _select_new_item(2); v.redraw=1;
    }
  }

  switch(v.active_item) {
    case 2: end_dialog=1;
      if (atoi(mancho)>0 && atoi(mancho)<32768) map_width=atol(mancho);
      if (atoi(malto)>0 && atoi(malto)<32768) map_height=atol(malto);
      v_finished=1; break;
...
int new_map(byte * mapilla) {
  ...
  if ((v_map=(struct tmapa *)malloc(sizeof(struct tmapa)))!=NULL) {
    ...
    } else { v_text=(char *)texto[45]; show_dialog(err0); free(v_map); }
...
void open_map(void) {
  ...
  if (sum && n==1) {
    x=0; sum=0; do { sum+=abs((int)original_palette[x]-(int)dac[x]); } while (++x<768);
            ...
            if (tipomapa) {
              if ((v_map=(struct tmapa *)malloc(sizeof(struct tmapa)))!=NULL) {
				...
				if ((v_map->map=(byte *)malloc(map_width*map_height+map_width))!=NULL) {
                  ...
                  x=1;
                  switch (tipomapa) {
                    case 1: descomprime_MAP(buffer,v_map->map,1); break;
                    case 2: descomprime_PCX(buffer,v_map->map,1); break;
                    case 3: descomprime_BMP(buffer,v_map->map,1); break;
                    case 4: x=descomprime_JPG(buffer,v_map->map,1,n); break;
                  }
                  ...
                  if(!x) {
                    ...
                    free(buffer);
                    fclose(f);
                    v_text=(char *)texto[44]; show_dialog(err0);
                    continue;
                  }
                  ...
                  if(fpg_code) v_map->TengoNombre=1;
                  v_map->map_width=map_width;
                  v_map->map_height=map_height;
...
void save_map(void) {
  int e,tipomapa;
  int an=ventana[v_window].an,al=ventana[v_window].al;
  char filename[255];
  FILE *f;
  ...
  if (strchr(input,' ')==NULL) {
    if ((f=fopen(full,"wb"))!=NULL) { // Se ha elegido uno
      ...
      } if (e==2) { v_text=(char *)texto[48]; show_dialog(err0); }
  ...
  if (!e) { // Debe cambiar la ruta y nombre del mapa/ventana guardado
    ...
    if (text_len(ventana[v_window].title)+3>an-20) {
      wwrite_in_box(ventana[v_window].ptr,an,an-19,al,4,2,0,ventana[v_window].title,c1);
      wwrite_in_box(ventana[v_window].ptr,an,an-19,al,3,2,0,ventana[v_window].title,c4);
    } else {
      wwrite(ventana[v_window].ptr,an,al,3+(an-20)/2,2,1,ventana[v_window].title,c1);
      wwrite(ventana[v_window].ptr,an,al,2+(an-20)/2,2,1,ventana[v_window].title,c4);
...
char cWork[5];
        ...
        sprintf(cWork,"%d",ventana[n].mapa->map_width);
        wwrite(v.ptr,an,al,4,20,0,(byte *)cWork,c3);

        wwrite(v.ptr,an,al,4,30,0,texto[134],c3);
        sprintf(cWork,"%d",ventana[n].mapa->map_height);
...
//Genera la tabla de grises
        ...
        {
                Progress((char *)texto[92],fy,map_height);
                x=(float)0;
...
//Gris=1 Convierte la imagen a grises
        ...
        Progress((char *)texto[92],map_height,map_height);
        ...
        if (fx<0)
                ...
                if (fx+vga_width>v.mapa->map_width)
                        fx=v.mapa->map_width-vga_width;
        fy=(v.mapa->zoom_cy-vga_height/2);
        if (fy<0)
                fy=0;
        else
                if(fy+vga_height>v.mapa->map_height)
...
void map_search2() {
  _process_items();
  switch(v.active_item) {
    case 3: v_accept=1; end_dialog=1; break;
    case 4: v_accept=0; end_dialog=1; break;
  }
}
...
void map_search() {
  ...
  v_accept=0; show_dialog(map_search0);
  ...
  for (y=0;y<map_height*itile;y+=itile) {
    Progress((char *)texto[404],y,map_height*itile);
  ...
  } Progress((char *)texto[404],map_height*itile,map_height*itile);
...
//Genera la tabla de RGB con Error de 1
        ...
        {
                Progress((char *)texto[92],y,map_height);
