...
void readmouse(void) {
  ...
  if (!mouse_in_window) {
    ...
    if (_mouse_x < 0) {
      _mouse_x = 0;
      n++;
    } else if (_mouse_x >= vga_width) {
      _mouse_x = vga_width - 1;
      n++;
    }
    if (_mouse_y < 0) {
      _mouse_y = 0;
      n++;
    } else if (_mouse_y >= vga_height) {
      _mouse_y = vga_height - 1;
      n++;
    }

    if (n)
      set_mouse(_mouse_x, _mouse_y);
  ...
  if (mouse->cursor & 1) {
    ...
    if (m_x < 0)
      m_x = 0;
    if (m_x >= vga_width)
      m_x = (float)(vga_width - 1);
    if (m_y < 0)
      m_y = 0;
    if (m_y >= vga_height)
      m_y = (float)(vga_height - 1);
